...
{
	public static function changeDateFormat($date, $input_format='Y-m-d H:i:s', $output_format = 'd M, Y') {
		try {
			$date = \Carbon\Carbon::createFromFormat($input_format, $date)->format($output_format);
		} 
		catch(\Exception $e) {
			//do nothing
			//echo $e->getMessage();
	...
	public static function tagDecode($tags) {
		return str_replace('#', '', $tags);
	}

	public static function getTagCloud($tags = NULL, $model='\App\Http\Controllers\CoreModules\Blog\BlogModel') {
		if(is_null($tags)) {
			$_tag = $model::where('is_active', 'yes')->pluck('tags')->toArray();
			$tags = implode(',', $_tag);
		}
